...
- Иногда инициировать разговор если есть повод
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(step_data[1], reply_markup=build_keyboard(step_data[2]))
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user or not user["onboarding_done"]:
        ...
        if step < len(ONBOARDING_STEPS):
            ...
            if next_step < len(ONBOARDING_STEPS):
                ...
                await update.message.reply_text(next_data[1], reply_markup=build_keyboard(next_data[2]))
            else:
    ...
    # Показываем сеть только если пользователь сам спросил и дал согласие на знакомства
    if user["open_to_meet"] != "Пока нет":
        all_users = get_all_users()
        other_users = [u for u in all_users if u[0] != telegram_id]
        if other_users and any(w in text.lower() for w in ["встреч", "познаком", "найди", "кто есть в сети"]):
            network_info = "\n\nЛюди в сети которые открыты к знакомствам:\n"
            for u in other_users[:5]:
                network_info += f"- {u[1]}, {u[5]}, {u[3]}, цель: {u[6]}\n"
            system += network_info
    ...
    try:
        ...
        reply = "Что-то пошло не так, попробуй ещё раз через минуту."
